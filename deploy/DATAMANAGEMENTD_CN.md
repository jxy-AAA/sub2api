...
当前主仓库 **不再包含** `datamanagementd` 源码目录，请先准备好已编译二进制（例如来自内部构建产物或独立仓库）。
...
仓库已提供服务文件：`deploy/sub2api-datamanagementd.service`
...
若 `sub2api` 运行在 Docker 容器中，需要把宿主机 socket 挂载到容器同路径：
...
建议在 `docker-compose.override.yml` 维护该挂载，避免覆盖主 compose 文件。
