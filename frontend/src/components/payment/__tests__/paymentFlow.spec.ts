...
describe('writePaymentRecoverySnapshot', () => {
  it('redacts sensitive fields before writing to storage', () => {
    const storage = {
      value: '',
    ...
    writePaymentRecoverySnapshot(storage, snapshot)

    expect(storage.value).not.toContain('cs_test_secret')
    expect(storage.value).not.toContain('resume-42')
    expect(storage.value).not.toContain('https://pay.example.com/session/42')
  })
})
