...
const managedAffiliatePermissions = ref<ManagedAffiliatePermissions>(emptyManagedAffiliatePermissions())
const hasManagedDownlineAccess = computed(() => hasManagedAffiliateAccess(managedAffiliatePermissions.value))
    ...
    h(
      ...
      [
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M19.5 14.25V6.75A2.25 2.25 0 0017.25 4.5H6.75A2.25 2.25 0 004.5 6.75v10.5A2.25 2.25 0 006.75 19.5h10.5A2.25 2.25 0 0019.5 17.25v-3M8.25 8.25h7.5M8.25 12h7.5m-7.5 3.75h4.5'
        })
      ]
    )
}
...
const flagManagedAffiliate = () => flagAffiliate() !== false && hasManagedDownlineAccess.value
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/apihub-guide', label: t('nav.apihubGuide'), icon: GuideIcon },
    { path: '/usage', label: t('nav.usage'), icon: ChartIcon, hideInSimpleMode: true },
    { path: '/available-channels', label: t('nav.availableChannels'), icon: ChannelIcon, hideInSimpleMode: true, featureFlag: flagAvailableChannels },
    { path: '/model-market', label: t('nav.modelMarket'), icon: GlobeIcon },
    ...
    { path: '/affiliate/managed', label: t('nav.downlineManagement'), icon: FolderIcon, hideInSimpleMode: true, featureFlag: flagManagedAffiliate },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/model-market', label: t('nav.modelMarket'), icon: PriceTagIcon, hideInSimpleMode: true },
    { path: '/admin/traces', label: t('nav.traces'), icon: ServerIcon, hideInSimpleMode: true },
    ...
    { path: '/admin/promo-codes', label: t('nav.promoCodes'), icon: GiftIcon, hideInSimpleMode: true },
    {
      ...
      children: [
        { path: '/admin/affiliates/daily-revenue', label: t('nav.affiliateInviteRecords'), icon: UsersIcon },
        { path: '/admin/affiliates/rebate-balances', label: t('nav.affiliateRebateRecords'), icon: OrderIcon },
        { path: '/admin/affiliates/management', label: t('nav.affiliateAgentManagement'), icon: UsersIcon },
        { path: '/admin/affiliates/monthly-archives', label: t('nav.affiliateMonthlyArchives'), icon: CreditCardIcon },
      ],
    },
    {
      path: '/admin/orders',
...
async function refreshManagedAffiliatePermissions() {
  if (!authStore.isAuthenticated || isAdmin.value || appStore.backendModeEnabled) {
    managedAffiliatePermissions.value = emptyManagedAffiliatePermissions()
    return
  }

  try {
    managedAffiliatePermissions.value = await getManagedAffiliatePermissions()
  } catch (error: any) {
    ...
    console.error('Failed to load managed affiliate permissions:', error)
    managedAffiliatePermissions.value = emptyManagedAffiliatePermissions()
  }
}
...
watch(
  [() => authStore.isAuthenticated, isAdmin, () => appStore.backendModeEnabled],
  () => {
    void refreshManagedAffiliatePermissions()
  },
  { immediate: true }
)
