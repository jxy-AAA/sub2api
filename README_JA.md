...
```bash
VERSION=vX.Y.Z
ARCHIVE="sub2api_${VERSION#v}_linux_amd64.tar.gz"
...
**スクリプトの動作内容:**
- バージョン同梱の `docker-compose.local.yml` と `.env.example` をコピー
- 可能なら `SUB2API_IMAGE_REF` をそのリリース版に固定
- `POSTGRES_PASSWORD`、`REDIS_PASSWORD`、`ADMIN_PASSWORD`、`JWT_SECRET`、`TOTP_ENCRYPTION_KEY` を生成
- secret は権限 `0600` の `.env` にだけ保存し、端末へ表示しない
- 作成するローカルディレクトリは `data/` のみで、PostgreSQL / Redis は名前付きボリュームを使用
...
|---------|-------------|-----------|----------|
| **docker-compose.local.yml** | `./data` + PostgreSQL/Redis の名前付きボリューム | 制御プレーンのみをアーカイブし、DB/Redis は個別にエクスポート | 本番環境、反復アップグレード |
| **docker-compose.yml** | 名前付きボリューム | Docker volume ワークフロー | シンプルな一体型デプロイ |

**推奨:** 本番では `docker-compose.local.yml` を優先してください。編集可能なアプリ設定/ログはローカルに残しつつ、`postgres_data` / `redis_data` が配布物へ混ざるのを防げます。

#### アクセス

既定では `127.0.0.1` のみで待ち受けます。公開する前に Caddy / Nginx / TLS リバースプロキシを前段に置いてください。

```bash
curl http://127.0.0.1:8080/health
```

管理者パスワードをログから読まないでください。`.env` に `ADMIN_PASSWORD` を明示設定してください。
...
`docker-compose.local.yml` を使用している場合も、移行するのは制御プレーンのみとし、PostgreSQL / Redis は別個にバックアップしてください:

```bash
# 移行元サーバーにて
docker compose -f docker-compose.local.yml down
cd ..
tar czf sub2api-control-plane.tar.gz sub2api-deploy/.env sub2api-deploy/docker-compose.local.yml sub2api-deploy/data/

# 新しいサーバーに転送
scp sub2api-control-plane.tar.gz user@new-server:/path/

# 移行先サーバーにて
tar xzf sub2api-control-plane.tar.gz
cd sub2api-deploy/
docker compose -f docker-compose.local.yml up -d
```
