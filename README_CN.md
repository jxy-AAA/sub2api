...
**脚本功能：**
- 复制随版本包提供的 `docker-compose.local.yml` 和 `.env.example`
- 尝试将 `SUB2API_IMAGE_REF` 固定到当前版本
- 生成 `POSTGRES_PASSWORD`、`REDIS_PASSWORD`、`ADMIN_PASSWORD`、`JWT_SECRET`、`TOTP_ENCRYPTION_KEY`
- 仅写入权限为 `0600` 的 `.env` 文件，不在终端打印 secret
- 只创建 `data/`；PostgreSQL 与 Redis 使用命名卷
...
| 版本 | 数据存储 | 迁移便利性 | 适用场景 |
|------|---------|-----------|---------|
| **docker-compose.local.yml** | `./data` + PostgreSQL/Redis 命名卷 | 仅打包控制面文件，数据库与 Redis 单独导出 | 生产环境、可重复升级 |
| **docker-compose.yml** | 命名卷 | Docker volume 工作流 | 简单一体化部署 |

**推荐：** 生产环境优先使用 `docker-compose.local.yml`。它保留可编辑的应用配置/日志目录，同时避免 `postgres_data` / `redis_data` 被打进发布包或迁移包。
...
不要再从日志中读取管理员密码；请在 `.env` 中显式设置 `ADMIN_PASSWORD`。
...
使用 `docker-compose.local.yml` 时，请只迁移控制面文件，并单独备份 PostgreSQL / Redis：
