...
func secureDialContext(dialFn func(ctx context.Context, network, address string) (net.Conn, error), resolver lookupIPFunc) func(ctx context.Context, network, address string) (net.Conn, error) {
	return func(ctx context.Context, network, address string) (net.Conn, error) {
		...
		var lastErr error
		for _, ip := range ips {
			conn, dialErr := dialFn(ctx, network, net.JoinHostPort(ip.String(), port))
			if dialErr == nil {
				return conn, nil
			}
			lastErr = dialErr
...
func resolveDialAddress(ctx context.Context, network, address string) (string, error) {
	host, port, err := net.SplitHostPort(address)
	if err != nil {
		return "", fmt.Errorf("invalid dial address %q: %w", address, err)
	}
	ips, err := resolveDialIPs(ctx, net.DefaultResolver.LookupIP, host)
	if err != nil {
		return "", err
	}
	if len(ips) == 0 {
		return "", fmt.Errorf("no validated ips resolved for %s", host)
	}
	return net.JoinHostPort(ips[0].String(), port), nil
}

func resolveDialIPs(ctx context.Context, resolver lookupIPFunc, host string) ([]net.IP, error) {
	host = strings.TrimSpace(host)
	if host == "" {
		return nil, errors.New("request host is empty")
	}
	if parsed := net.ParseIP(host); parsed != nil {
		if isDisallowedUpstreamIP(parsed) {
			return nil, fmt.Errorf("resolved ip %s is not allowed", parsed.String())
		}
	...
	for _, ip := range ips {
		if ip == nil || isDisallowedUpstreamIP(ip) {
			continue
		}
...
func (s *httpUpstreamService) getClientEntry(proxyURL string, accountID int64, accountConcurrency int, markInFlight bool, enforceLimit bool) (*upstreamClientEntry, error) {
	...
	// 缓存未命中或需要重建，创建新客户端
	settings := s.resolvePoolSettings(isolation, accountConcurrency)
	transport, err := buildUpstreamTransport(settings, parsedProxy, s.shouldValidateResolvedIP())
	if err != nil {
		s.mu.Unlock()
...
func buildUpstreamTransport(settings poolSettings, proxyURL *url.URL, validateDialTarget bool) (*http.Transport, error) {
	dialer := &net.Dialer{}
	dialContext := dialer.DialContext
	var targetResolver proxyutil.TargetResolver
	if validateDialTarget {
		targetResolver = resolveDialAddress
		if proxyURL == nil {
			dialContext = secureDialContext(dialContext, net.DefaultResolver.LookupIP)
		}
	}
	...
	if err := proxyutil.ConfigureTransportProxyWithTargetResolver(transport, proxyURL, targetResolver); err != nil {
		return nil, err
	}
...
func buildUpstreamTransportWithTLSFingerprint(settings poolSettings, proxyURL *url.URL, profile *tlsfingerprint.Profile, validateDialTarget bool) (*http.Transport, error) {
	transport := &http.Transport{
		MaxIdleConns:          settings.maxIdleConns,
		MaxIdleConnsPerHost:   settings.maxIdleConnsPerHost,
		MaxConnsPerHost:       settings.maxConnsPerHost,
	...
	var targetResolver func(ctx context.Context, network, addr string) (string, error)
	if validateDialTarget {
		targetResolver = resolveDialAddress
	}
	if proxyURL == nil {
		...
		if validateDialTarget {
			baseDialer = secureDialContext((&net.Dialer{}).DialContext, net.DefaultResolver.LookupIP)
		}
		...
		case "socks5", "socks5h":
			...
			socks5Dialer := tlsfingerprint.NewSOCKS5ProxyDialerWithTargetResolver(profile, proxyURL, targetResolver)
			transport.DialTLSContext = socks5Dialer.DialTLSContext
		case "http", "https":
			// HTTP/HTTPS 代理：使用 HTTPProxyDialer（CONNECT 隧道）
			slog.Debug("tls_fingerprint_transport_http_connect", "proxy", proxyURL.Host)
			if validateDialTarget {
				return nil, fmt.Errorf("%s proxy cannot bind validated upstream IP", scheme)
			}
			httpDialer := tlsfingerprint.NewHTTPProxyDialerWithTargetResolver(profile, proxyURL, targetResolver)
