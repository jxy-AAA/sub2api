		...
		res, err := r.sql.ExecContext(ctx, `
			WITH maintenance AS (
				...
				WHERE u.created_at < $1
					AND NOT EXISTS (
						SELECT 1 FROM affiliate_distribution_usage_settlements s
						WHERE s.usage_log_id = u.id
					)
					AND NOT EXISTS (
		...
		if month.Before(cutoffMonth) {
			...
			checkSQL := fmt.Sprintf(`
				SELECT EXISTS (
					...
					WHERE EXISTS (
						SELECT 1 FROM affiliate_distribution_usage_settlements s
						WHERE s.usage_log_id = ul.id
					)
					OR EXISTS (
						SELECT 1 FROM affiliate_distribution_usage_jobs j
						WHERE j.usage_log_id = ul.id
					)
					LIMIT 1
				)
			`, pq.QuoteIdentifier(name))
			if err := scanSingleRow(ctx, r.sql, checkSQL, nil, &hasAffiliateRefs); err != nil {
				return err
			}
			if hasAffiliateRefs {
				continue
