...
func ValidateHTTPURL(raw string, allowInsecureHTTP bool, opts ValidationOptions) (string, error) {
	...
	if err := validateHostSafety(host, opts.AllowPrivate); err != nil {
...
func ValidateURLFormat(raw string, allowInsecureHTTP bool) (string, error) {
	return ValidateURLFormatWithOptions(raw, allowInsecureHTTP, false)
}

func ValidateURLFormatWithOptions(raw string, allowInsecureHTTP bool, allowPrivate bool) (string, error) {
	// 最小格式校验：保证 URL 可解析且 scheme 合规；默认阻断私网/回环/链路本地/metadata 风险主机
	trimmed := strings.TrimSpace(raw)
	if trimmed == "" {
		return "", errors.New("url is required")
	}
	...
	if err := validateHostSafety(strings.ToLower(host), allowPrivate); err != nil {
		return "", err
	}
...
func validateHostSafety(host string, allowPrivate bool) error {
	if allowPrivate {
		return nil
	}
	if isBlockedHost(host) {
		return errors.New("private host is not allowed")
	}
	return nil
}
