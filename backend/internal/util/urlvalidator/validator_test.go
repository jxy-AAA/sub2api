...
func TestValidateURLFormat(t *testing.T) {
	...
	if _, err := ValidateURLFormat("https://127.0.0.1", false); err == nil {
		t.Fatalf("expected private host to fail by default")
	}
...
func TestValidateURLFormatWithOptions_AllowsPrivateOnlyWhenExplicitlyEnabled(t *testing.T) {
	if _, err := ValidateURLFormatWithOptions("https://localhost", false, false); err == nil {
		t.Fatalf("expected localhost to be blocked when allowPrivate=false")
	}
	if _, err := ValidateURLFormatWithOptions("https://localhost", false, true); err != nil {
		t.Fatalf("expected localhost to pass with explicit allowPrivate, got %v", err)
	}
}
