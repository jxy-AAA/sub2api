...
func (h *AuthHandler) userDTOWithRootAdmin(ctx context.Context, user *service.User) *dto.User {
	out := dto.UserFromService(user)
	if out == nil || h == nil || h.userService == nil || user == nil || !user.IsAdmin() {
		return out
	}
	firstAdmin, err := h.userService.GetFirstAdmin(ctx)
	if err != nil {
		return out
	}
	out.IsRootAdmin = firstAdmin != nil && firstAdmin.ID == user.ID
	return out
}
...
func (h *AuthHandler) respondWithTokenPair(c *gin.Context, user *service.User) {
	...
	if err != nil {
		...
		response.Success(c, AuthResponse{
			AccessToken: token,
			TokenType:   "Bearer",
			User:        h.userDTOWithRootAdmin(c.Request.Context(), user),
		})
		return
	}
...
func (h *AuthHandler) setRefreshTokenCookie(c *gin.Context, refreshToken string) {
	...
	value := strings.TrimSpace(refreshToken)
	if value == "" {
		return
	}
...
func (h *AuthHandler) GetCurrentUser(c *gin.Context) {
	...
	if user.IsAdmin() {
		...
		isRootAdmin = firstAdmin != nil && firstAdmin.ID == user.ID
	}
...
// ValidateAffiliateCodeRequest validates an affiliate code.
type ValidateAffiliateCodeRequest struct {
	Code string `json:"code" binding:"required"`
}

// ValidateAffiliateCodeResponse is returned by affiliate code validation.
...
func (h *AuthHandler) ValidateAffiliateCode(c *gin.Context) {
	...
	summary, err := h.authService.ValidateAffiliateCode(c.Request.Context(), req.Code)
	if err != nil {
...
// ValidateInvitationCode validates an invitation code before registration.
...
func (h *AuthHandler) RefreshToken(c *gin.Context) {
	...
	if refreshToken == "" {
		if value, err := c.Cookie(authRefreshTokenCookieName); err == nil {
...
func (h *AuthHandler) Logout(c *gin.Context) {
	...
	if req.RefreshToken == "" {
		if value, err := c.Cookie(authRefreshTokenCookieName); err == nil {
			req.RefreshToken = strings.TrimSpace(value)
		}
	}
