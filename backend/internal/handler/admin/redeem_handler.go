...
func (h *RedeemHandler) GetStats(c *gin.Context) {
	...
	byType := map[string]int64{
		...
		service.RedeemTypeAffiliateBalance: 0,
	}

	for {
		codes, total, err := h.adminService.ListRedeemCodes(c.Request.Context(), page, pageSize, "", "", "", "id", "desc")
		if err != nil {
			response.ErrorFrom(c, err)
			return
		}
		...
		for i := range codes {
			...
			byType[code.Type] = byType[code.Type] + 1
		}
	...
	response.Success(c, gin.H{
		...
		"by_type": gin.H{
			service.RedeemTypeBalance:          byType[service.RedeemTypeBalance],
			service.RedeemTypeConcurrency:      byType[service.RedeemTypeConcurrency],
			service.RedeemTypeSubscription:     byType[service.RedeemTypeSubscription],
			service.RedeemTypeInvitation:       byType[service.RedeemTypeInvitation],
			service.RedeemTypeAffiliateBalance: byType[service.RedeemTypeAffiliateBalance],
		},
	})
}
