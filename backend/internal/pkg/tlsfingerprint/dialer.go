...
type HTTPProxyDialer struct {
	profile        *Profile
	proxyURL       *url.URL
	targetResolver func(ctx context.Context, network, addr string) (string, error)
}
...
func NewHTTPProxyDialer(profile *Profile, proxyURL *url.URL) *HTTPProxyDialer {
	return NewHTTPProxyDialerWithTargetResolver(profile, proxyURL, nil)
}

func NewHTTPProxyDialerWithTargetResolver(profile *Profile, proxyURL *url.URL, targetResolver func(ctx context.Context, network, addr string) (string, error)) *HTTPProxyDialer {
	return &HTTPProxyDialer{profile: profile, proxyURL: proxyURL, targetResolver: targetResolver}
}
...
func NewSOCKS5ProxyDialer(profile *Profile, proxyURL *url.URL) *SOCKS5ProxyDialer {
	return NewSOCKS5ProxyDialerWithTargetResolver(profile, proxyURL, nil)
}

func NewSOCKS5ProxyDialerWithTargetResolver(profile *Profile, proxyURL *url.URL, targetResolver func(ctx context.Context, network, addr string) (string, error)) *SOCKS5ProxyDialer {
	return &SOCKS5ProxyDialer{profile: profile, proxyURL: proxyURL, targetResolver: targetResolver}
}
...
func (d *SOCKS5ProxyDialer) DialTLSContext(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	if d.targetResolver != nil {
		resolvedAddr, err := d.targetResolver(ctx, network, addr)
		if err != nil {
			return nil, err
		}
		targetAddr = resolvedAddr
	}
	slog.Debug("tls_fingerprint_socks5_establishing_tunnel", "target", targetAddr)
...
func (d *HTTPProxyDialer) DialTLSContext(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	slog.Debug("tls_fingerprint_http_proxy_sending_connect", "target", targetAddr)
