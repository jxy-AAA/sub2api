...
var (
	...
	ErrAPIKeyTooWeak      = infraerrors.BadRequest("API_KEY_TOO_WEAK", "api key is too weak; use a higher-entropy key")
...
func (s *APIKeyService) ValidateCustomKey(key string) error {
	...
	if isWeakCustomAPIKey(key) {
		return ErrAPIKeyTooWeak
	}

	return nil
}

func isWeakCustomAPIKey(key string) bool {
	normalized := strings.ToLower(strings.TrimSpace(key))
	if normalized == "" {
		return true
	}
	if strings.Count(normalized, string(normalized[0])) == len(normalized) {
		return true
	}
	...
	if maxRepeatRun >= len(normalized)-2 {
		return true
	}
