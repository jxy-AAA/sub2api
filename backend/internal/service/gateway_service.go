...
func safeHeaderValueForLog(key string, v string) string {
	...
	switch key {
	case "authorization", "proxy-authorization", "x-api-key", "x-goog-api-key", "cookie", "set-cookie", "x-access-token", "x-refresh-token":
...
func sanitizeJSONValueForLog(key string, value any) any {
	...
	case map[string]any:
		...
		for childKey, childValue := range typed {
			sanitized[childKey] = sanitizeJSONValueForLog(childKey, childValue)
		}
		return sanitized
	case []any:
		sanitized := make([]any, 0, len(typed))
		for _, childValue := range typed {
			sanitized = append(sanitized, sanitizeJSONValueForLog(key, childValue))
		}
		return sanitized
	case string:
		if len(typed) > 512 {
...
func formatGatewayBodyForLog(body []byte) string {
	if len(body) == 0 {
		return "  (empty)\n"
	}
	...
	if err := json.Unmarshal(body, &payload); err != nil {
		return fmt.Sprintf("  (non-json omitted; bytes=%d sha256=%x)\n", len(body), sum)
	}
	sanitized := sanitizeJSONValueForLog("", payload)
	encoded, err := json.MarshalIndent(sanitized, "  ", "  ")
	if err != nil {
		return fmt.Sprintf("  (json omitted; bytes=%d sha256=%x)\n", len(body), sum)
	}
	if len(encoded) > maxOutputLen {
		encoded = append(encoded[:maxOutputLen], []byte("\n  ...(truncated)")...)
	}
	return fmt.Sprintf("  bytes=%d sha256=%x\n  %s\n", len(body), sum, encoded)
}
...
type GatewayService struct {
	...
	affiliateDistributionSettlementService AffiliateDistributionUsageSettlementService
}
	...
	if writer, ok := repo.(usageLogBestEffortWriter); ok {
		...
		if usageLog.ID == 0 {
			if _, err := repo.Create(usageCtx, usageLog); err != nil {
				logger.LegacyPrintf(logKey, "Resolve usage log id failed: %v", err)
				return false
			}
		}
		return usageLog.ID > 0
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	if writeUsageLogBestEffort(ctx, s.usageLogRepo, usageLog, "service.gateway") {
		settleAffiliateDistributionUsageBestEffort(ctx, s.affiliateDistributionSettlementService, usageLog, "service.gateway")
	}
...
func (s *GatewayService) debugLogGatewaySnapshot(tag string, headers http.Header, body []byte, extra map[string]string) {
	...
	// 3. body（敏感字段脱敏 + 摘要化）
	fmt.Fprint(&buf, "--- body ---\n")
	fmt.Fprint(&buf, formatGatewayBodyForLog(body))
