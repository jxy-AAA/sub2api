...
func (s *SubscriptionService) AssignOrExtendSubscription(ctx context.Context, input *AssignSubscriptionInput) (*UserSubscription, bool, error) {
	...
	if dbent.TxFromContext(ctx) == nil && s.entClient != nil {
		...
		ownTx = true
		workCtx = dbent.NewTxContext(ctx, tx)
		defer func() { _ = tx.Rollback() }()
	}

	existingSub, err := s.loadExistingSubscriptionForAssign(workCtx, input.UserID, input.GroupID)
	if err != nil {
	...
	if existingSub != nil {
		...
		if err := s.userSubRepo.ExtendExpiry(workCtx, existingSub.ID, newExpiresAt); err != nil {
			return nil, false, fmt.Errorf("extend subscription: %w", err)
		}
		if existingSub.Status != SubscriptionStatusActive {
			if err := s.userSubRepo.UpdateStatus(workCtx, existingSub.ID, SubscriptionStatusActive); err != nil {
		...
		if input.Notes != "" {
			...
			if err := s.userSubRepo.UpdateNotes(workCtx, existingSub.ID, newNotes); err != nil {
		...
		s.invalidateAssignSubscriptionCaches(input.UserID, input.GroupID)
		sub, err := s.userSubRepo.GetByID(workCtx, existingSub.ID)
		return sub, true, err
	}

	sub, err := s.createSubscription(workCtx, &AssignSubscriptionInput{
		UserID:       input.UserID,
...
func (s *SubscriptionService) getExistingSubscriptionForAssign(ctx context.Context, userID, groupID int64) (*UserSubscription, error) {
	if s.entClient != nil {
		if tx := dbent.TxFromContext(ctx); tx != nil {
			query := tx.Client().UserSubscription.Query().
			...
			return &UserSubscription{
				...
				Notes:     subscriptionNotesValue(entity.Notes),
			}, nil
		}
	}
...
func (s *SubscriptionService) loadExistingSubscriptionForAssign(ctx context.Context, userID, groupID int64) (*UserSubscription, error) {
	if s.entClient != nil {
		if tx := dbent.TxFromContext(ctx); tx != nil {
			query := tx.Client().UserSubscription.Query().
				Where(usersubscription.UserIDEQ(userID), usersubscription.GroupIDEQ(groupID))
...
func (s *SubscriptionService) invalidateAssignSubscriptionCaches(userID, groupID int64) {
	s.InvalidateSubCache(userID, groupID)
	if s.billingCacheService == nil {
		return
	}
...
func subscriptionNotesValue(notes *string) string {
	if notes == nil {
		return ""
	}
	return *notes
