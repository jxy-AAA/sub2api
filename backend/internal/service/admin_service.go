...
func (s *adminServiceImpl) GetUserUsageStats(ctx context.Context, userID int64, period string) (any, error) {
	if s.usageLogRepo == nil {
		return nil, infraerrors.New(http.StatusNotImplemented, "USAGE_STATS_UNAVAILABLE", "usage stats repository not available")
	}
	now := time.Now().UTC()
	start := usagePeriodStart(now, period)
	filters := usagestats.UsageLogFilters{UserID: userID, StartTime: &start, EndTime: &now}
	stats, err := s.usageLogRepo.GetStatsWithFilters(ctx, filters)
	if err != nil {
		return nil, err
	}
...
func usagePeriodStart(now time.Time, period string) time.Time {
