...
func TestOpenAIValidateUpstreamBaseURLDisabledBlocksPrivateHostByDefault(t *testing.T) {
	cfg := &config.Config{
		Security: config.SecurityConfig{
			URLAllowlist: config.URLAllowlistConfig{Enabled: false},
		},
	...
	if _, err := svc.validateUpstreamBaseURL("https://127.0.0.1"); err == nil {
		t.Fatalf("expected private host to be rejected when allow_private_hosts is false")
	}
}

func TestOpenAIValidateUpstreamBaseURLDisabledAllowsPrivateHostWhenOptedIn(t *testing.T) {
	cfg := &config.Config{
		Security: config.SecurityConfig{
			URLAllowlist: config.URLAllowlistConfig{
				Enabled:           false,
	...
	normalized, err := svc.validateUpstreamBaseURL("https://127.0.0.1")
	if err != nil {
		t.Fatalf("expected private host allowed when allow_private_hosts is true, got %v", err)
	}
	if normalized != "https://127.0.0.1" {
