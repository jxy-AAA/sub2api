...
func (s *PaymentService) createOrderInTx(ctx context.Context, req CreateOrderRequest, user *User, plan *dbent.SubscriptionPlan, cfg *PaymentConfig, orderAmount, limitAmount, feeRate, payAmount float64, sel *payment.InstanceSelection) (*dbent.PaymentOrder, error) {
	...
	if err := s.checkDailyLimit(ctx, tx, req.UserID, limitAmount, cfg.DailyLimit, true); err != nil {
		return nil, err
	}
...
func (s *PaymentService) checkDailyLimit(ctx context.Context, tx *dbent.Tx, userID int64, amount, limit float64, includePending bool) error {
	if limit <= 0 {
		return nil
	}
	...
	for _, o := range orders {
		used += paymentOrderLimitAmount(o.OrderType, o.Amount, o.PayAmount)
	}
	if includePending {
		pendingOrders, pendingErr := tx.PaymentOrder.Query().
			Where(
			...
			All(ctx)
		if pendingErr != nil {
			return fmt.Errorf("query pending daily usage: %w", pendingErr)
		}
		for _, pending := range pendingOrders {
			used += paymentOrderLimitAmount(pending.OrderType, pending.Amount, pending.PayAmount)
		}
	}
