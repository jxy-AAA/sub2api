...
func (s *PaymentService) GetDashboardStats(ctx context.Context, days int) (*DashboardStats, error) {
	...
	paidStatuses := []paymentorder.Status{
		paymentorder.Status(OrderStatusCompleted),
		paymentorder.Status(OrderStatusPaid),
		paymentorder.Status(OrderStatusRecharging),
	}
...
func (s *PaymentService) writeAuditLog(ctx context.Context, oid int64, action, op string, detail map[string]any) {
	if err := s.writeAuditLogWithClient(ctx, s.entClient, oid, action, op, detail); err != nil {
